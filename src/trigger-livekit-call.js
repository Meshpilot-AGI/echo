...
import { canonicalBrandIdForShopSync } from './lib/hub-context.js';
...
// Normalise a shop / store identifier into a safe object-store path segment.
// Recordings are organised store-wise so each client's calls live under their
// own R2 prefix: <RECORDING_PREFIX><shop>/<room>.mp4. Falls back to "unknown"
// so a missing shop never collapses the key to the bucket root.
function shopKeySegment(shop) {
  const slug = String(shop || '')
    .trim()
    .toLowerCase()
    .replace(/\.myshopify\.com$/, '')   // normalise Shopify domains → bare handle
    .replace(/[^a-z0-9_-]/g, '-')        // keep only key-safe chars
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
  return slug || 'unknown';
}

// Detect the LiveKit Cloud "concurrent egress sessions limit exceeded" error so
// we can back off and retry rather than permanently drop the recording.
function isEgressConcurrencyError(err) {
  const msg = String(err?.message || err || '').toLowerCase();
  return msg.includes('concurrent egress') || msg.includes('egress sessions limit');
}

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// Start a room-composite audio egress into the configured cloud bucket.
// Recordings are stored store-wise: <RECORDING_PREFIX><shop>/<room>.mp4.
// Non-fatal: if anything fails, we log and continue with the call (better to
// lose a training recording than lose the customer). The concurrent-egress
// plan limit is transient (a long call overlaps the scheduler's next tick), so
// we retry that specific error with exponential backoff before giving up.
async function startAudioEgress(room, shop) {
  ...
  // Prefer the canonical brand_id (e.g. "urban-classics") over the raw Shopify
  // handle (e.g. "f51039") so recordings sort by store the operator recognises.
  // The boot-time hub audit populates this cache for every configured shop;
  // fall back to the shop handle on a cache miss (hub-detached / new shop).
  const canonicalBrand = canonicalBrandIdForShopSync(shop);
  const storeSeg = shopKeySegment(canonicalBrand || shop);
  const filepath = `${RECORDING_PREFIX}${storeSeg}/${room}.mp4`;
  ...
  // Retry only the concurrency-limit error: a finishing call frees a slot
  // within seconds. Backoff: 5s, 15s, 30s (total ~50s window).
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      console.log(`[egress] started room=${room} shop=${storeSeg} egress_id=${info.egressId} → ${RECORDING_BACKEND}://${RECORDING_BUCKET}/${filepath}`);
      return info;
    } catch (err) {
      if (isEgressConcurrencyError(err) && attempt < backoffs.length) {
        const wait = backoffs[attempt];
        console.warn(`[egress] concurrency limit hit for room=${room} — retry ${attempt + 1}/${backoffs.length} in ${wait}ms`);
        await sleep(wait);
        continue;
      }
 ...
 * @param {string} params.profile         - Agent profile id (e.g. 'cod-confirm', 'appointment-remind')
 ...
 *                                          For cod-confirm: { shop, entityRef=orderId,
...
export async function triggerLivekitCall(params) {
  ...
  const profileId = profile || 'cod-confirm';
  ...
  setTimeout(() => {
    startAudioEgress(room, identity.shop).then(info => {
  ...
  const sip = await sipClient.createSipParticipant(
    ...
    room,
    {
      ...
      participantAttributes: {
        ...
        // Per-store branding only. Do NOT fall back to the single-tenant
        // STORE_NAME env here — in a multi-store deployment that would make a
        // store with unresolved branding wrongly introduce itself as another
        // brand. Leave empty; renderContext supplies a neutral "our store".
        ...
        // with the cod-confirm prompt; new profiles use payload.* directly.
