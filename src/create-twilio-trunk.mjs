  ...
  if (lkTrunk) {
    ...
    lkTrunk = await lk.createSipOutboundTrunk(
      ...
      [TWILIO_FROM_NUMBER],
      {
        ...
        // SIP_TRANSPORT_TLS — pairs with Secure=true on the Twilio trunk so SIP
        // signalling is encrypted end-to-end.
        transport:       3,
        // SRTP is NOT automatic. A Twilio Secure Trunk requires encrypted
        // MEDIA; without mediaEncryption the trunk returns
        // "488 secure media (SRTP) required" and every INVITE bounces before it
        // reaches Twilio (no CDR). 2 = SIP_MEDIA_ENCRYPT_REQUIRE (force SRTP so media actually flows).
        mediaEncryption: 2,
      },
    );
...
console.log('  sudo systemctl restart cod-confirm-agent');
console.log('');
