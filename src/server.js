 ...
 *
 * Profile-mounted endpoints (delegated to profiles/cod-confirm/index.js):
...
// housekeeping (e.g. cod-confirm tagging the Shopify order 'cod-no-answer')
...
// ─── Profile mount ────────────────────────────────────────────────────
// Must run BEFORE the global express.json — the Shopify webhook (cod-confirm)
...
app.get('/health', async (_req, res) => {
  ...
  res.json({
    ...
    cod_confirm_runtime: COD_CONFIRM_RUNTIME,
    ...
    retell_agent_configured: Boolean(
...
//   payload         — profile-specific context (cod-confirm: customer_name,
...
app.post('/calls/dispatch', requireToolAuth, async (req, res) => {
  ...
  // The IST DND window applies to India (cod-confirm). The `concierge` profile
  // is international (US/+1) — IST hours are wrong for it, so don't roll its
  // scheduledAt (the scheduler's dispatch gate also skips DND for concierge).
  const isConciergeReq = (b.profile || '') === 'concierge';
  const callWindow = getStoreCallWindow(b.shop);
  let scheduledAt = b.scheduledAt
    ? new Date(b.scheduledAt)
    : (isConciergeReq ? new Date() : computeScheduledAt(new Date(), CALL_DELAY_MS, callWindow));
  if (Number.isNaN(scheduledAt.getTime())) {
    return res.status(400).json({ ok: false, error: 'invalid scheduledAt' });
  }
  // Roll any time that lands in the store's DND window forward to the next safe
  // slot — India only. (The scheduler re-checks at dispatch too — defence in depth.)
  if (!isConciergeReq) scheduledAt = adjustForDnd(scheduledAt, callWindow);

  // The schema still has shop+orderId as the natural key for backwards
  // compatibility with cod-confirm. For generic dispatches we synthesize a
...
//
// Currently hard-coded to the cod-confirm row shape (shop + shopify_order_id).
...
app.post('/webhook/livekit/turn', requireToolAuth, async (req, res) => {
  ...
  // SEC-aikido-prisma-operator-injection (2026-07-27): the checks above are
  // truthiness-only, so a JSON body can smuggle an *object* into a scalar
  // where-clause — `{"room_name": {"not": ""}}` is truthy and Prisma reads it
  // as a filter operator, turning the updateMany below into a write against
  // every CallAttempt row. Pin the types before they reach a where clause.
  if (typeof b.room_name !== 'string' || !Number.isInteger(b.turn_index)) {
    return res.status(400).json({ ok: false, error: 'room_name must be a string and turn_index an integer' });
  }
...
app.post('/webhook/vobiz/call-event', async (req, res) => {
  // Opt-in shared-secret gate (2026-06-10, bug-2). Vobiz has no signature
  // scheme, and this endpoint is public. When VOBIZ_WEBHOOK_SECRET is set we
  // require a matching x-vobiz-secret header (timing-safe); when unset we
  // allow but warn, so enabling it is a config-only change that can't break
  // the live integration before Vobiz is configured to send the header.
  const vobizSecret = process.env.VOBIZ_WEBHOOK_SECRET || '';
  if (vobizSecret) {
    const got = String(req.get('x-vobiz-secret') || '');
    const a = Buffer.from(got);
    const b = Buffer.from(vobizSecret);
    if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
      return res.status(401).json({ ok: false, error: 'bad_vobiz_secret' });
    }
  } else {
    console.warn('[vobiz-event] VOBIZ_WEBHOOK_SECRET unset — endpoint is unauthenticated');
  }
  const ev = req.body || {};
  const eventType = String(ev.event || ev.type || ev.Event || '').toLowerCase();
  const callUuid  = ev.CallUUID || ev.call_uuid || ev.callUuid || ev.calluuid || '';
  const sipCallId = ev.sip_call_id || ev.sipCallId || '';

  console.log('[vobiz-event]', eventType, 'CallUUID:', callUuid || '-', 'sipCallId:', sipCallId || '-', JSON.stringify(ev).slice(0, 400));

  // Store CallUUID on call-start events for later recording matching.
  if (callUuid && (eventType === 'ring' || eventType === 'startapp' || eventType === 'answer')) {
    try {
      const matchId = sipCallId || callUuid;
      // BUG-9 (2026-06-10): the old updateMany stamped sipCallId onto EVERY
      // unlinked attempt in the last 10 min with no phone/shop filter, so
      // under concurrent calls one CallUUID linked to many unrelated attempts
      // (which then mis-fed the recording matcher). Link the SINGLE most-recent
      // unlinked attempt whose phone matches the event's destination number.
      const evTo = String(
        ev.to_number || ev.to || ev.To || ev.destination || ev.dest ||
        ev.from_number || ''
      ).trim();
      if (!evTo) {
        // No destination number on the event — do NOT blanket-link. The
        // recording matcher falls back to phone matching, so skipping is safe.
        console.warn('[vobiz-event] no destination number; skipping CallUUID back-link');
      } else {
        const norm = evTo.replace(/^\+/, '');
        const phoneCandidates = [...new Set([evTo, norm, `+${norm}`])];
        ...
        if (attempt) {
          await prisma.callAttempt.update({
            where: { id: attempt.id },
            data: { sipCallId: matchId },
          });
          console.log(`[vobiz-event] linked CallUUID ${callUuid} to CallAttempt ${attempt.id}`);
        }
      }
    } catch (e) {
      console.warn('[vobiz-event] link failed:', e.message);
    }
  }

  // Recording ready — download from Vobiz, upload to R2.
  if (eventType === 'recording.completed' || eventType === 'recordingcompleted') {
    try {
      const result = await processVobizRecording(ev, prisma);
      console.log('[vobiz-event] recording processed:', result);
    } catch (e) {
      console.error('[vobiz-event] recording processing failed:', e.message);
    }
  }
...
async function onFinalFail(row, reason) {
  const profileId = row.profile || 'cod-confirm';
  try {
...
app.listen(PORT, '127.0.0.1', async () => {
  ...
  console.log(`[glitch-voice] COD CONFIRM RUNTIME: ${COD_CONFIRM_RUNTIME}`);
  {
    let perShopCount = 0;
