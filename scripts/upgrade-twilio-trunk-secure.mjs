...
const newLkTrunk = await lk.createSipOutboundTrunk(
  ...
  [TWILIO_FROM_NUMBER],
  {
    ...
    // MUST set SRTP too — a Twilio Secure Trunk requires encrypted MEDIA, not
    // just TLS signaling. Without this the trunk returns
    // "488 secure media (SRTP) required" and every INVITE bounces before it
    // even reaches Twilio (no CDR). 2 = SIP_MEDIA_ENCRYPT_REQUIRE (force SRTP so media actually flows).
    mediaEncryption: 2,
  },
);
console.log(`✓ Created LiveKit trunk ${newLkTrunk.sipTrunkId} on TLS + SRTP.`);
...
console.log('─────────────────────────────────────────────────');
console.log('\nThen restart:');
console.log('  sudo systemctl restart cod-confirm cod-confirm-agent');
